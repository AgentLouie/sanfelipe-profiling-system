    ...
    <div className="relative mt-0 space-y-4 animate-in fade-in duration-500">
      ...
      {deleteModal.isOpen && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md" onClick={() => setDeleteModal({ isOpen: false, residentId: null, name: '' })} />
          <div className="relative z-10 bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <p className="text-sm text-stone-500 mt-2">
              Are you sure you want to delete <span className="font-bold text-stone-800">{deleteModal.name}</span>? This action cannot be undone.
            </p>
            <div className="flex gap-3 mt-6">
              <button onClick={() => setDeleteModal({ isOpen: false, residentId: null, name: '' })} className="flex-1 py-2.5 rounded-xl border border-stone-200 text-stone-600 text-sm font-semibold hover:bg-stone-50 transition-colors" disabled={isDeleting}>
                Cancel
              </button>
              <button onClick={confirmDelete} className="flex-1 py-2.5 rounded-xl bg-red-600 text-white text-sm font-semibold hover:bg-red-700 transition-colors flex items-center justify-center gap-2" disabled={isDeleting}>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-stone-100 p-4 md:p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          {userRole === 'admin' && (
            <div className="relative">
              <select value={selectedBarangay} onChange={handleBarangayFilter} className="w-full appearance-none pl-10 pr-10 py-3 bg-stone-50 border border-stone-100 rounded-xl outline-none text-sm focus:ring-2 focus:ring-rose-500/20">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-stone-50">
              {residents.length === 0 && !loading ? (
                <tr><td colSpan="6" className="py-20 text-center text-stone-400 italic">No records found.</td></tr>
              ) : (
                residents.map((r) => (
                  <Fragment key={r.id}>
                    <tr
                    ...
                    >
                      ...
                      <td className="py-4 px-6">
                        ...
                        <p className="text-xs font-bold text-rose-500 uppercase">{r.occupation || "N/A"}</p>
                      </td>

                      {/* 2. BIRTHDATE */}
                      <td className="py-4 px-6">
                        <div className="flex flex-col">
                          <span className="text-sm font-semibold text-stone-700">{formatDate(r.birthdate)}</span>
                          <span className="text-xs text-stone-400 font-medium">
                            {r.birthdate ? `(${calculateAge(r.birthdate)} yrs)` : ''}
                          </span>
                        </div>
                      </td>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 p-3 bg-white/80 backdrop-blur-md border border-stone-200/60 rounded-3xl shadow-sm">
        <div className="flex items-center gap-4 order-2 md:order-1">
          <div className="flex items-center gap-2 bg-stone-100/50 px-3 py-1.5 rounded-2xl border border-stone-200/40">
            ...
            <div className="relative flex items-center">
              <select value={itemsPerPage} onChange={handleLimitChange} className="appearance-none bg-transparent pr-5 text-sm font-bold text-stone-700 outline-none cursor-pointer focus:text-rose-600 transition-colors">
                <option value={10}>10</option>
                <option value={20}>20</option>
                <option value={50}>50</option>
                <option value={100}>100</option>
              </select>
              <ChevronDown size={12} className="absolute right-0 top-1 text-stone-400 pointer-events-none" />
            </div>
          </div>
        ...
        <div className="flex items-center gap-3 order-1 md:order-2">
          ...
          <div className="flex items-center gap-1.5">
            <button disabled={currentPage === 1 || loading} onClick={() => setCurrentPage(prev => prev - 1)} className="w-10 h-10 flex items-center justify-center rounded-2xl border border-stone-200 text-stone-600 hover:border-rose-200 hover:bg-rose-50 hover:text-rose-600 disabled:opacity-20 disabled:hover:bg-transparent disabled:hover:border-stone-200 transition-all duration-200 active:scale-95">
              <ChevronLeft size={20} />
            </button>
            <div className="hidden sm:flex items-center gap-1.5">
              {paginationRange?.map((pageNumber, idx) => {
                if (pageNumber === "...") return <span key={idx} className="w-8 text-center text-stone-300 font-black">···</span>;
                const isActive = pageNumber === currentPage;
                return (
                  <button key={idx} onClick={() => setCurrentPage(pageNumber)} className={`w-10 h-10 flex items-center justify-center rounded-2xl text-sm font-bold transition-all duration-200 active:scale-95 ${isActive ? "bg-stone-900 text-white shadow-lg shadow-stone-200 scale-105" : "text-stone-500 hover:bg-stone-100 border border-transparent hover:border-stone-200"}`}>
                    {pageNumber}
                  </button>
                );
              })}
            </div>
            <button disabled={currentPage === totalPages || loading} onClick={() => setCurrentPage(prev => prev + 1)} className="w-10 h-10 flex items-center justify-center rounded-2xl border border-stone-200 text-stone-600 hover:border-rose-200 hover:bg-rose-50 hover:text-rose-600 disabled:opacity-20 transition-all duration-200 active:scale-95">
